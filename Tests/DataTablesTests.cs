...
namespace Tests
{
    public class DataTablesTests
    {
        ...
        #region SearchFormattedName

        [Fact]
        public void TestSearchFormattedNames()
        {
            TestSearchFormattedName("a", "a%");
            TestSearchFormattedName(" a", " a%");
            TestSearchFormattedName("a b", "a b%");
            TestSearchFormattedName("a,b", "a%, b%");
            TestSearchFormattedName("a,  b", "a%, b%");
            TestSearchFormattedName("a,       b", "a%, b%");
            TestSearchFormattedName("a b,c", "a b%, c%");
        }

        private static void TestSearchFormattedName(string searchText, string expectedResult)
        {
            var search = new Search { value = searchText };
            Assert.Equal(expectedResult, search.SearchFormattedName);
        }
        ...
        private static void TestParseUnitNumber(string unit, int expectedNumber, string expectedLetters)
        {
        ...
        private static void TestGetUnitNumberSortKey(string unit, string expectedSortKey)
        {
